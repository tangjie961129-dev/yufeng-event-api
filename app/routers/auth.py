...
def _get_wecom_access_token() -> str:
    corpsecret = settings.WECOM_SECRET or settings.WECOM_APP_SECRET
    if not settings.WECOM_CORP_ID or not corpsecret:
        return ""
    try:
...
def _fetch_wecom_join_way_url() -> str:
    ...
    # Prefer dynamic WeCom customer-group join way over the older static QR fallback.
    if settings.WECOM_GROUP_JOIN_WAY_ID:
...
def _build_user_info(user: User) -> UserInfo:
    ...
    return UserInfo(
        id=user.id,
        user_no=user.phone if user.phone else f"{35000 + int(user.id)}",
        ...
        match_records=items,
        test_remaining=user.test_remaining or 0,
    )
...
async def wx_login(req: WxLoginRequest, db: Session = Depends(get_db)):
    ...
    if not user:
        user = User(openid=openid, nickname=req.nickname or "", avatar_url=req.avatar_url or "", test_remaining=3)
        db.add(user)
        db.commit()
        db.refresh(user)
    else:
...
@router.get("/user/test-remaining")
def get_test_remaining(user: User = Depends(get_current_user)):
    """查询当前用户剩余免费测试次数"""
    return {"test_remaining": user.test_remaining or 0}
...
@router.put("/user/profile", response_model=UserInfo)
def put_profile(
    req: ProfileUpdateRequest,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """更新用户信息（小程序编辑资料页使用 PUT /api/user/profile）。"""
    return _apply_profile_update(req, user, db)
...
def update_profile(
    ...
    """更新用户信息（兼容旧版 POST /api/user/update-profile）。"""
    return _apply_profile_update(req, user, db)


def _apply_profile_update(req: ProfileUpdateRequest, user: User, db: Session) -> UserInfo:
    nickname = (req.nickname or "").strip()
    avatar_url = (req.avatar_url or "").strip()
    phone = (req.phone or "").strip()
    city = (req.city or "").strip()
    age = int(req.age or 0)
