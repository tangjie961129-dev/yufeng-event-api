...
@router.get("/registrations", response_model=AdminOrderListResponse)
def admin_list_registrations(
    ...
    """管理员查看所有报名记录，可按活动ID筛选"""
    query = db.query(EventRegistration, Event, User).join(
    ...
    if keyword:
        query = query.filter(or_(
            Event.title.ilike(f"%{keyword}%"),
            User.nickname.ilike(f"%{keyword}%"),
            EventRegistration.remark.ilike(f"%{keyword}%"),
        ))
...
@router.put("/registrations/{reg_id}/remark")
def admin_update_registration_remark(
    reg_id: int,
    body: dict,
    admin: AdminUser = Depends(get_current_admin),
    db: Session = Depends(get_db),
    ...
    if remark is None:
        raise HTTPException(400, "缺少 remark 字段")

    reg.remark = str(remark)
    db.commit()
    return {"message": "备注已更新", "id": reg.id, "remark": reg.remark}


@router.get("/registrations/events")
def admin_list_registration_events(
    admin: AdminUser = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
    """返回有报名记录的活动列表（用于下拉筛选）"""
    events = db.query(Event).join(
        EventRegistration, EventRegistration.event_id == Event.id
    ).filter(
        EventRegistration.status.in_(["pending", "paid", "verified"])
    ).distinct().order_by(Event.start_time.desc()).all()
    return [{"id": e.id, "title": e.title, "start_time": str(e.start_time)[:16]} for e in events]

@router.put("/events/{event_id}/display-count")
def admin_update_display_count(
    event_id: int,
    req: dict,
    admin: AdminUser = Depends(get_current_admin),
    db: Session = Depends(get_db),
    ...
    if not event:
        raise HTTPException(404, "活动不存在")
    display_count = req.get("display_registrant_count")
    if display_count is not None:
        event.display_registrant_count = int(display_count) if int(display_count) >= 0 else None
    db.commit()
    return {"message": "显示报名数已更新", "display_registrant_count": event.display_registrant_count}
