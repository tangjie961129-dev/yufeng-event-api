...
class AgentProfile(Base):
    ...
    agent_type = Column(String(30), default="promoter", index=True)  # regional_partner/promoter
    parent_agent_user_id = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)
    service_region = Column(String(100), default="", index=True)
    direct_commission_rate = Column(DECIMAL(5, 2), default=50.00)
    management_base_rate = Column(DECIMAL(5, 2), default=10.00)
    management_bonus_rate = Column(DECIMAL(5, 2), default=0.00)
    risk_level = Column(String(30), default="normal", index=True)
    compliance_status = Column(String(30), default="normal", index=True)
    status = Column(String(30), default="pending", index=True)
    approved_by = Column(Integer, ForeignKey("admin_users.id"), nullable=True)
    approved_at = Column(DateTime(timezone=True), nullable=True)
    reject_reason = Column(Text, default="")
    bind_channel = Column(String(30), default="")
    identity_label = Column(String(50), default="推广员")
    note = Column(Text, default="")
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class ReferralBinding(Base):
    ...
    source_scene = Column(String(255), default="", index=True)
    source_platform = Column(String(30), default="mini_program", index=True)
    landing_page = Column(String(255), default="")
    campaign_code = Column(String(100), default="")
    binding_context_json = Column(Text, default="{}")
    status = Column(String(30), default="pending", index=True)
    attribution_status = Column(String(30), default="active", index=True)
    lock_days = Column(Integer, default=180)
...
class AgentTeamManagementBonusLedger(Base):
    """区域合伙人季度团队管理奖冻结池（默认10%，季度结算）"""
    __tablename__ = "agent_team_management_bonus_ledgers"

    id = Column(Integer, primary_key=True, index=True)
    regional_partner_user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    promoter_user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    invited_user_id = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)
    source_order_type = Column(String(50), default="official_product", index=True)
    source_order_id = Column(String(100), default="", index=True)
    source_commission_ledger_id = Column(Integer, ForeignKey("agent_commission_ledgers.id"), nullable=True, index=True)
    base_amount = Column(DECIMAL(10, 2), default=0)
    base_rate = Column(DECIMAL(5, 2), default=10.00)
    performance_rate = Column(DECIMAL(5, 2), default=0.00)
    bonus_amount = Column(DECIMAL(10, 2), default=0)
    quarter = Column(String(20), nullable=False, index=True)
    settlement_status = Column(String(30), default="frozen", index=True)
    settlement_batch_no = Column(String(100), default="")
    ...
    reversed_at = Column(DateTime(timezone=True), nullable=True)
    reverse_reason = Column(Text, default="")
    business_key = Column(String(120), nullable=False, unique=True, index=True)
    note = Column(Text, default="")
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
