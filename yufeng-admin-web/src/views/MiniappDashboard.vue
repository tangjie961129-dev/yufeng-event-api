...
const loadData = async () => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.detail || '加载小程序业务概览失败')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div v-loading="loading" class="miniapp-dashboard">
    <section class="miniapp-hero">
      <div>
        <p>MINIAPP BUSINESS ADMIN</p>
        <h1>小程序业务后台</h1>
        <span>这里保留会员、活动、订单、课程问卷、页面装修、分销佣金等小程序业务管理；客服大脑和企微运营请回到总后台。</span>
      </div>
      <router-link to="/dashboard" class="back-ops">返回总后台</router-link>
    </section>
...
<style scoped>
.miniapp-dashboard { display: flex; flex-direction: column; gap: 20px; }
.miniapp-hero {
  display: flex;
...
.miniapp-hero p { margin: 0 0 8px; color: #2563eb; font-size: 12px; font-weight: 800; letter-spacing: .12em; }
.miniapp-hero h1 { margin: 0 0 8px; color: #111827; }
.miniapp-hero span { color: #64748b; }
.back-ops { align-self: flex-start; padding: 10px 14px; border-radius: 999px; background: #0f172a; color: #fff; text-decoration: none; font-size: 13px; }
@media(max-width: 767px) { .miniapp-hero { flex-direction: column; } }
</style>
