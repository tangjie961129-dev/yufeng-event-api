...
const menuGroups = [
  {
    ...
    items: [
      ...
      { label: '运营任务台', path: '/ops/console', icon: Operation },
      { label: '企微回补', path: '/wecom-backfill', icon: Connection },
      { label: '会员标签', path: '/member-tags', icon: PriceTag },
      { label: '客服话术库', path: '/ops/scripts', icon: ChatDotRound, disabled: true },
      { label: '朋友圈推送台', path: '/ops/moments', icon: Promotion, disabled: true },
      { label: '指令动作中心', path: '/ops/actions', icon: Operation, disabled: true },
    ],
  },
  {
    title: '小程序业务后台',
    ...
    items: [
      ...
      { label: '合作推广', path: '/cooperation', icon: Promotion },
...
const isOpsRoute = computed(() => {
  return route.path === '/dashboard' || route.path.startsWith('/ops') || ['/wecom-backfill', '/member-tags'].includes(route.path)
})
